...
/// Version of the built-in `upt-base` virtual dependency.
pub const UPT_BASE_API_VERSION: &str = "1.0.0";
...
/// A compiled-in module of the `upt-base` registry.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
...
pub fn builtin_modules() -> Vec<BuiltinModule> {
    ...
    vec![
        ...
        module("upt.proto.i2c", CapabilitySet::default(), "I2C 芯片协议"),
        module(
            "upt.proto.microwire",
            CapabilitySet::default(),
            "MicroWire 芯片协议",
...
pub fn load_plugin_state(root: &Path) -> HashMap<String, bool> {
    ...
    let Ok(value) = text.parse::<toml::Value>() else {
        eprintln!(
            "[upt-plugin] ignoring malformed plugin state file {}",
            path.display()
        );
        return HashMap::new();
    };
    ...
    if let Some(table) = value.get("plugins").and_then(toml::Value::as_table) {
        for (name, enabled) in table {
            if let Some(enabled) = enabled.as_bool() {
                ...
                eprintln!(
                    "[upt-plugin] ignoring non-boolean enabled value for plugin '{name}' in {}",
                    path.display()
                );
            }
        }
...
impl PluginManager {
    ...
    ///
    /// Dependency names are first looked up in the built-in `upt-base`
    /// registry (see [`builtin_modules`]) and then among loaded plugins.
    /// `upt-base` itself remains a virtual meta dependency: it is checked
    /// against [`UPT_BASE_API_VERSION`] and expands implicitly to the whole
    /// builtin registry, so no individual builtin entries are appended to the
    /// returned list for a `upt-base` dependency.
    ///
    ...
    pub fn resolve_dependencies(&self, name: &str) -> Result<Vec<String>, String> {
        ...
        fn visit(
            ...
            for dep in &plugin.manifest.dependencies {
                ...
                if dep.name == "upt-base" {
                    let available = semver::Version::parse(UPT_BASE_API_VERSION)
                        .expect("UPT_BASE_API_VERSION must be valid semver");
                    if !requirement.matches(&available) {
                        return Err(format!(
                            "plugin '{name}' requires upt-base '{}' but built-in version is {UPT_BASE_API_VERSION}",
                            dep.requirement
                        ));
                    }
...
"#,
    ...
    #[test]
    fn dependency_resolution_upt_base_unknown_and_unsatisfied() {
        ...
        let resolved = manager
            .resolve_dependencies("only-upt-base")
            .expect("upt-base ^1");
        assert_eq!(resolved, vec!["only-upt-base"]);
        ...
        let err = manager
            .resolve_dependencies("unsat-upt-base")
            .expect_err("unsatisfied upt-base must fail");
        assert!(err.contains("requires upt-base '^2'"), "{err}");

        let err = manager
            .resolve_dependencies("dep-version")
            .expect_err("unsatisfied plugin version must fail");
        assert!(err.contains("requires only-upt-base '~0.9'"), "{err}");
    }
    ...
    fn builtin_dependency_versions_are_checked() {
        ...
        let resolved = manager
            ...
            .expect("upt.hal ^1 must resolve");
        assert_eq!(resolved, vec!["uses-hal-v1"]);

        let err = manager
            .resolve_dependencies("uses-hal-v2")
            .expect_err("upt.hal ^2 must fail");
        assert!(
            err.contains("依赖 upt.hal 需要 ^2，当前内置版本为 1.0.0"),
    ...
    fn load_scans_plugins_builtin_directory() {
        ...
        write_builtin_manifest(&root, "upt.hal", &builtin_manifest("upt.hal", "adapter"));

        let manager = PluginManager::load(&root);

        assert_eq!(manager.errors, Vec::<(String, String)>::new());
        assert_eq!(manager.plugins.len(), 1);
        assert_eq!(manager.plugins[0].manifest.name, "upt.hal");
    ...
    fn builtin_cold_plugins_start_enabled_but_can_be_disabled_in_state() {
        ...
        let manifest = r#"
[package]
name = "upt.hal.ch34x_dll"
version = "1.0.0"
plugin_api = 1
kind = "adapter"
layer = "cold"
entry = "upt_ch34x_sidecar_dll"
provider = "builtin"
...
max_freq_khz = 60000
"#;
        write_builtin_manifest(&root, "upt.hal.ch34x_dll", manifest);
        ...
        assert_eq!(state.get("upt.hal.ch34x_dll"), Some(&true));

        let mut disabled = manager;
        disabled
            .disable("upt.hal.ch34x_dll")
...
"#;
    ...
    fn boot_check_reports_missing_required_plugins() {
        let manager = PluginManager {
            plugins: vec![loaded_plugin("upt.hal", "1.0.0", Vec::new())],
            errors: Vec::new(),
        };
        ...
        assert_eq!(
            ...
            vec![
                "upt.tauri".to_string(),
                "upt.chipdb".to_string(),
                "upt.tauri.hexview".to_string(),
                "upt.proto".to_string(),
            ]
        );
    ...
    fn boot_check_reports_duplicate_required_plugin_as_invalid() {
        ...
        write_manifest(&root, "upt.hal", &builtin_manifest("upt.hal", "adapter"));
        write_builtin_manifest(&root, "upt.hal", &builtin_manifest("upt.hal", "adapter"));
        ...
        let boot = manager.boot_check();
        assert_eq!(boot.invalid, vec!["upt.hal".to_string()]);
        assert!(!boot.missing.contains(&"upt.hal".to_string()));
    ...
    fn boot_check_reports_parse_error_required_plugin_as_invalid() {
        let manager = PluginManager {
            plugins: Vec::new(),
            errors: vec![(
                "/plugins/builtin/upt.hal/unipkg.toml".to_string(),
    ...
    fn required_plugin_cannot_be_disabled_and_enable_is_noop() {
        ...
        assert!(
            err.contains("required plugin 'upt.hal' cannot be disabled"),
            "unexpected error: {err}"
        );
        assert!(!manager.plugins[0].enabled);
