...
use crate::app_ops::operations::BlankCheckResult;
...
fn load_chip_lib(state: State<'_, Mutex<AppState>>) -> Result<String, String> {
    ...
    log_info("load_chip_lib: 开始加载芯片库");
    let base = exe_dir();
...
async fn connect_serprog(
    ...
    let mut s = state.lock().map_err(|e| e.to_string())?;
    log_info(&format!("connect_serprog: port={port}"));
    ...
    s.serprog = Some(dev);
    log_info(&format!("connect_serprog: 已连接 {info}"));
    Ok(format!("已连接: {}", info))
}
...
async fn chip_erase(
    ...
    if let Ok(message) = &result {
        log_info(&format!("chip_erase: {message}"));
    }
    result
}
...
async fn read_chip(
    ...
    log_info(&format!("read_chip: 读取完成 {} 字节", data.len()));
...
async fn write_chip(
    ...
    if let Ok(message) = &result {
        log_info(&format!("write_chip: {message}"));
    }
    result
}
...
async fn verify_chip(
    ...
    if let Ok(message) = &result {
        log_info(&format!("verify_chip: {message}"));
    }
    result
}
...
/// Attach every UI command to a Tauri builder. Keeping this function next
/// to the command definitions avoids exposing the command wrapper macros.
pub fn attach_handler(builder: tauri::Builder<tauri::Wry>) -> tauri::Builder<tauri::Wry> {
