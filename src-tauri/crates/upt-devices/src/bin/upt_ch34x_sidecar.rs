...
// upt-hal sidecar transport, so it only depends on the `upt-devices` crate
...
use upt_devices::ch34x::{self, Ch34xDevice, Ch34xSettings, ChipKind, DeviceMode};
...
fn probe_devices() -> Vec<Value> {
    match ch34x::DllHal::enumerate() {
        Ok(devices) => devices
            ...
            .collect(),
        Err(e) => {
            eprintln!("[upt_ch34x_sidecar] probe enumeration error: {e}");
            Vec::new()
        }
    }
}
...
fn probe_devices() -> Vec<Value> {
    eprintln!("[upt_ch34x_sidecar] probe enumeration unavailable: no CH34X HAL backend configured");
    Vec::new()
}
...
fn handle_spi_transact(id: u64, op: &Value, device: &mut Ch34xDevice) -> Value {
    ...
    match spi_transact(device, &request) {
        Ok(data) => {
            eprintln!(
                "[upt_ch34x_sidecar] spi_transact write_len={} read_len={}",
...
fn main() {
    ...
    eprintln!("[upt_ch34x_sidecar] ready");

    loop {
        let payload = match read_frame(&mut reader) {
            Ok(payload) => payload,
            Err(e) => {
                eprintln!("[upt_ch34x_sidecar] read error: {e}");
                break;
            }
        };
        ...
        let request: Value = match serde_json::from_slice(&payload) {
            Ok(request) => request,
            Err(e) => {
                eprintln!("[upt_ch34x_sidecar] invalid JSON request: {e}");
                continue;
            }
        };

        let Some(id) = request.get("id").and_then(Value::as_u64) else {
            eprintln!("[upt_ch34x_sidecar] request without numeric id ignored");
            continue;
        };
        ...
        let response = match method {
            "handshake" => {
                eprintln!("[upt_ch34x_sidecar] handshake");
                success_response(
                    id,
                    json!({
                        "protocol_version": 1,
                        "name": "upt.hal.ch34x-sidecar",
                        "version": "1.0.0",
                        "plugin_api": 1,
                        "capabilities": {
                            "spi": {
            ...
            "probe" => {
                eprintln!("[upt_ch34x_sidecar] probe");
                let devices = probe_devices();
                eprintln!(
                    "[upt_ch34x_sidecar] probe found {} CH34X device(s)",
                    devices.len()
                );
            ...
            "close" => {
                ...
                match session.as_ref() {
                    Some(current) if current.id == sid => {
                        eprintln!("[upt_ch34x_sidecar] close session={sid}");
                        session = None;
                ...
                match session.as_mut() {
                    Some(current) if current.id == sid => {
                        ...
                        match op_name {
                            ...
                            "gpio_set" => {
                                eprintln!("[upt_ch34x_sidecar] gpio_set denied");
                                error_response(
                                    id,
        ...
        if let Err(e) = write_frame(&mut writer, &response_payload) {
            eprintln!("[upt_ch34x_sidecar] write error: {e}");
            break;
        }
        if let Err(e) = writer.flush() {
            eprintln!("[upt_ch34x_sidecar] flush error: {e}");
            break;
        }
    }

    eprintln!("[upt_ch34x_sidecar] exiting");
}
