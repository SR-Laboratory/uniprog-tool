//! `uni.chipdb` sidecar process.
//!
//! Speaks the same 4-byte little-endian length + UTF-8 JSON protocol as the
//! other `uni_hal` sidecars. Debug logs go to stderr only.
...
fn handle_load(id: u64, params: &Value) -> Value {
    ...
    match Chiplib::load_auto(&xml_path, &bin_path) {
        Ok(lib) => {
            ...
            eprintln!("[uni.chipdb] loaded {} entries from {}", count, bin_path);
        ...
        Err(e) => {
            eprintln!("[uni.chipdb] load failed: {e}");
...
fn main() {
    ...
    eprintln!("[uni.chipdb] ready");

    loop {
        let payload = match read_frame(&mut reader) {
            Ok(payload) => payload,
            Err(e) => {
                eprintln!("[uni.chipdb] read error: {e}");
                break;
            }
        };
        ...
        let request: Value = match serde_json::from_slice(&payload) {
            Ok(request) => request,
            Err(e) => {
                eprintln!("[uni.chipdb] invalid JSON request: {e}");
                continue;
            }
        };

        let Some(id) = request.get("id").and_then(Value::as_u64) else {
            eprintln!("[uni.chipdb] request without numeric id ignored");
            continue;
        };
        let method = request.get("method").and_then(Value::as_str).unwrap_or("");
        let params = request.get("params").cloned().unwrap_or_else(|| json!({}));

        eprintln!("[uni.chipdb] method={method}");

        let response = match method {
            "handshake" => {
                eprintln!(
                    "[uni.chipdb] handshake name={} version={}",
        ...
        if let Err(e) = write_frame(&mut writer, &response_payload) {
            eprintln!("[uni.chipdb] write error: {e}");
            break;
        }
        if let Err(e) = writer.flush() {
            eprintln!("[uni.chipdb] flush error: {e}");
            break;
        }
    }

    eprintln!("[uni.chipdb] exiting");
}
