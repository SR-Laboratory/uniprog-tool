...
// L1 UI contribution slot. `upt.tauri.hexview` is a required plugin, so the
...
const HEXVIEW_URL = navigator.userAgent.includes('Windows')
  ? 'http://unipkg.localhost/upt.tauri.hexview/'
  : 'unipkg://localhost/upt.tauri.hexview/'
...
<template>
  <div class="plugin-hex-host">
    <iframe
      ...
      class="plugin-frame"
      :src="HEXVIEW_URL"
      title="upt.tauri.hexview"
      @load="onFrameLoad"
    />
    <div v-if="loadFailed" class="plugin-error">
      <p>HexViewer 插件未能加载（upt.tauri.hexview）</p>
